...
def crossover():
    ...
    if (code_temp.find("pos+1")!= -1):
        code_temp = code_temp.replace("pos+1", "pos*1")
    else:
        code_temp = code_temp.replace("pos*1", "pos+1")
...
def mutate(code_temp):
    ...
    #swapping order of code line 4
    if(code_temp.find("not found and pos")!=-1):
        code_temp = code_temp.replace("pos < len(alist)", "not found")
        code_temp = code_temp.replace("not found", "pos < len(alist)", 1)
    else:
        code_temp = code_temp.replace("not found", "pos < len(alist)")
        code_temp = code_temp.replace("pos < len(alist)", "not found", 1)
    return code_temp
# use some
def fitness(code_temp):
    ...
    #exec(code_temp)
    #test example
    # as we may have "malformed" offspring, we use try clause to keep program running without stop the program
    try:
